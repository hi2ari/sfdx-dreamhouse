		...
		withEnv(["HOME=${env.WORKSPACE}"]) 
		{
			...
			stage('Authorize DevHub') {
				if (isUnix()) {
					...
					rc = bat returnStatus: true, script: "\"${toolbelt}\" force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile \"${jwt_key_file}\" --setdefaultdevhubusername -a HubOrg --instanceurl ${SFDC_HOST}"
			...
			stage('Create Scratch Org'){
				...
				if (isUnix()) {
				  rmsg = sh returnStdout: true, script: "${toolbelt} force:org:create --definitionfile config/enterprise-scratch-def.json --json --setdefaultusername"
					...
					rmsg = bat returnStdout: true, script: "\"${toolbelt}\" force:org:create --targetdevhubusername HubOrg --setdefaultusername --definitionfile config/project-scratch-def.json --json"
				}
				printf rmsg
